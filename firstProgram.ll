...
define internal fastcc double @f(double %x) #0 {
entry:
  %0 = tail call fastcc double @lambda(double %x)
  ret double %0
}

; Function Attrs: nounwind readnone
define internal fastcc double @lambda(double %x) #0 {
entry:
...
; Function Attrs: nounwind readnone
define internal fastcc double @fib(double %x1, double %y) #0 {
entry:
...
if.else:                                          ; preds = %entry
  ...
  %2 = tail call fastcc double @fib(double %1, double %y)
...
entry:
  %0 = tail call fastcc double @fib(double 1.000000e+01, double 2.342340e+05)
  %1 = tail call fastcc double @f(double 2.342340e+05)
  %2 = fadd double %0, %1
  ret double %2
}
